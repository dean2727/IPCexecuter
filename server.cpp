/*------------------------------------------------------------
 * server.cpp: Receiver process that is typically the child
 *   process of the sender. Functions similarly to client.cpp
...
void process_file_request (RequestChannel* rc, char* request) {
	...
	//cout << "The request is " << request << " and f.length is " << f.length << endl;
	string filename = request + sizeof (filemsg);
	filename = "BIMDC/" + filename; // adding the path prefix to the requested file name
	//cout << "Server received request for file " << filename << " and f.length is " << f.length << endl;

	// client is asking for file size
	...
	//cout << "nbytes formed from response = " << response << ", f.length = " << f.length <<", and fp = " << fp << ", and f.offset = " << f.offset << endl;
	...
	remaining length is < buffercap of the client */
	//cout << "nbytes is "<< nbytes << " and f.length is " << f.length << endl;
	assert (nbytes == f.length);
	//cout << "On server side, writing back\n"; 
...
// Execute a certain routine depending on what kind of request was received
...
int main(int argc, char *argv[]) {
	//cout << "Howdy! I am the child server process. My ID is " << getpid() << endl;
