/*-----------------------------------------------------------
 * SHMreqchannel.h: SHMQ class, inherits from RequestChannel
 *   and uses shared memory segments and semaphores
 * Dean Orenstein
 * UIN: 127008117
 * CSCE 313-909
 * Date: 10/04/2020
...
#include <semaphore.h>
#include <sys/mman.h>  // For shared memory and mmap
#include <string>
#include "common.h"
...
public:
	SHMQ (string _name, int _len) : name (_name), len (_len) {
		...
		// Set length to 1024, necesarry because the default is 0
		ftruncate (fd, len);
